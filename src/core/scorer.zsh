...
LIMIT ${limit};"
    ...
    if [[ -n "$results" ]]; then
        ...
        while IFS= read -r line; do
            [[ -n "$seq_override" && "$line" == *"${_SAGE_SEP}${seq_cmd}" ]] && continue
            reply+=("$line")
        done <<< "$results"
    fi
}
...
_sage_score_candidate() {
    # Force C numeric locale so float formatting uses '.' as the decimal
    # mark. Without this, locales like sv_SE produce '0,98' which breaks
    # downstream numeric parsing. LC_ALL is needed too: if the user
    # exports LC_ALL=sv_SE.UTF-8 it overrides LC_NUMERIC entirely.
